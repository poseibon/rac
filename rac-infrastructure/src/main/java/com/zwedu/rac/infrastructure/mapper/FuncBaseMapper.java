...
public interface FuncBaseMapper {
    @Delete({
        "delete from tb_func",
    ...
    @Insert({
        "insert into tb_func (en_name, cn_name, ",
        "biz_line_id, content, ",
        ...
        "#{bizLineId,jdbcType=BIGINT}, #{content,jdbcType=VARCHAR}, ",
    ...
    int insert(FuncPo record);

    @InsertProvider(type=FuncSqlSupporter.class, method="insertSelective")
    @Options(useGeneratedKeys=true,keyProperty="id")
    int insertSelective(FuncPo record);
    ...
    FuncPo selectByPrimaryKey(Long id);

    @UpdateProvider(type=FuncSqlSupporter.class, method="updateByPrimaryKeySelective")
    int updateByPrimaryKeySelective(FuncPo record);
    ...
    int updateByPrimaryKey(FuncPo record);
}